...
### 目的
考察と同じニュース群から、割り当てられたアンカーニュースに直接結び付く具体的な内装製品企画を提案する。各国・地域ごとに2件程度。

### プロンプトテンプレート

```
以下の対象ニュースから、{国名}市場向けの内装製品企画アイデアを2件提案してください。
...
【生成ルール】
1. 各アイデアには以下を含める：
   - タイトル（20文字以内）：製品コンセプトを端的に表現
   - 説明（120〜180文字）：製品の特徴、ターゲット、差別化ポイント、うれしさを含める
2. 実現可能性を考慮しつつ、革新性も追求する
3. アンカーニュース固有の部品・材料・技術・課題を企画の中核にし、別ジャンルへ飛躍しない
4. シート、座席、座面、背もたれ、ヘッドレストなどの座席製品は企画対象外とする
5. インパネ、コンソール、ドアトリム、HMI、照明、安全表示、内装材料、音響・静粛などを優先する
6. 日本語で出力する
7. 画像生成用のプロンプトも併記する（英語、50〜100 words）
...
### 目的
企画アイデアのコンセプト画像をGemini APIで生成する。参照ニュースに画像URLがある場合は、その画像もデザイン文脈の参考として添付する。
...
4. **画像生成**: 各アイデアのimagePromptと参照ニュース画像をGemini APIへ送り、画像を作成
