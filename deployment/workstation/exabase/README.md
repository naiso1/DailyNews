...
認証確認で会話画面に到達できた場合は、その確認中に更新された状態を暗号化して原子的に保存し、続く生成にも更新後の状態を渡します。認証確認に失敗した場合は既存の `auth.bin` を上書きしません。保存処理も既存のworkerロック内で実行します。この対応は認証確認時の更新保存に限定しており、共有生成エンジン内で発生する後続の更新保存や、サービス側の認証期限の延長は行いません。
