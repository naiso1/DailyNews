...
`image_generation.reference_mode` は省略時に従来のテキストのみです。`"source-image"` を設定すると、企画が引用する記事のうち画像URLのある最初の1件を参考にします。画像はURL文字列を送るのではなく、取得した実ファイルを1枚だけ添付します。参照写真は形状と取付位置の補助とし、企画を最優先する、車種・ブランド・背景・配色・構図・カモフラージュを模倣しない指示を追加します。サービス側の参照強度を数値指定する機能は確認していません。

取得時には公開HTTP(S) URL・リダイレクト先、10MBの上限、JPEG/PNG/WebPの実形式、256px以上の寸法、単一フレーム、画像の完全な復号を確認します。認証付きURL・ローカルネットワーク宛て・SVG/HTMLなどを拒否します。画像なしや取得・検証失敗時は、その案を従来のテキスト入力で生成します。通常処理の結果には取得失敗の安全なコードと `fallback: "text-only"` を記録し、記事公開を停止しません。添付後・送信後に不明な状態となった場合は、自動で別の要求を送り直しません。

参考画像のモード、記事ID、記事URL、画像URL、SHA256、形式・寸法を非公開ジョブのmanifestに記録し、画像内容と出典をcache keyに含めます。既存のテキストだけのcache keyは変更しません。`attachment.json` と `attachment-accepted.png` は、送信前のサムネイル受理の証跡です。`phase.json` の送信開始記録を保存してから送信ボタンを1回だけ押し、添付画像自身を生成結果として取り違えないよう除外して回収します。

既存の掲載画像を保った比較には、日付・企画ID・引用記事IDをすべて明示します。1回につき新規生成は最大1枚で、`--publish` は併用できません。

```powershell
& $runtimePython -B -m dailynews.exabase --edition exterior --pilot-idea-id 35 --date 2026-09-20 --reference-source-id jp32
```

比較結果は `exabase-reference-pilot-last-result.json`、入力と出力は `exabase-jobs/<key>/` に保存し、既存の掲載画像や通常処理の結果ファイルを上書きしません。同一入力の完了画像は再利用し、送信後の結果不明は従来同様に人の確認まで停止します。画像を変更した比較を繰り返すと生成数が増えるため、比較対象と枚数を決めて実施します。

2026-09-21に、外装9月20日号のID35（スプリッター）とID44（バンパー加飾）で各1枚、計2枚の非公開比較を行いました。2枚とも写真の添付と生成を確認しましたが、ID35は別ブランド風のグリル・徽章が現れ、ID44も既存のテキスト版より優れているとまでは判断できませんでした。この実測に基づき、掲載画像と通常処理のテキスト入力を維持し、`reference_mode` は通常設定で有効化していません。利用する場合は、企画対象の部位が写る写真を人が選び、形状・取付位置・企画との一致・似すぎを確認します。市場記事の汎用車両写真や、記事対象とは別の車種の写真を無条件で添付しません。

1枚目はサービス側で生成が完了したものの、生成画像のaltが一時ファイル名だったため初期の取得判定が拒否しました。再送せず該当会話の完成画像を回収し、同一keyのDONE結果として保存しています。判定は入力写真を除外しつつ生成完了メッセージ以降の画像を認める方式に修正し、2枚目は通常経路で自動保存まで確認しました。比較記録は `runtime/exterior/reference-pilot/comparison.md` です。

完成画像が会話に残る場合に限り、workerの `--recover-reference` は明示した既存会話からの回収を行えます。既存requestのkey・prompt・referenceImage・outputDirと、実画面で確認した `conversationPath` を標準入力のJSONに渡します。元の企画文と添付名の一致を検証し、新規会話、アップロード、送信を行いません。回収後は同じkeyのDONE結果を通常の画像検証に通して再利用します。送信結果が確認できないまま `--generate` を直接呼び直さないでください。
...
| `REFERENCE_DOWNLOAD_FAILED` / `REFERENCE_URL_REJECTED` / `REFERENCE_INVALID_IMAGE` | 通常処理はテキストのみへ戻る。明示した比較試験では送信せず停止 |
| `REFERENCE_NOT_ACCEPTED` / `REFERENCE_UNSUPPORTED` | 添付画面の変更を確認。生成送信前に停止する |
| `REFERENCE_CHANGED` | 入力写真や出典が変わった。現在の参照情報を確認 |
...
- 既定の参照情報は企画文と元記事のタイトル・要約です。共有軽量エンジンには添付画像の引数がないため、写真参照はプロジェクト専用拡張で処理します。通常処理の設定で有効化するまで、元記事の写真は送信しません。
