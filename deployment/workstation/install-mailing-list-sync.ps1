...
Register-ScheduledTask -TaskName $task -Action $action -Trigger $trigger -Principal $principal -Settings $settings -Description "Refresh exterior opt-in recipients every two minutes; no news collection or mail sending." -Force | Out-Null
Write-Output "EXTERIOR_MAILING_LIST_SYNC_INSTALLED"
