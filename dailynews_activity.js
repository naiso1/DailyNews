...
function injectActivityStyles() {
  ...
  style.textContent = `
    ...
    .engagement-row{--engagement-thumb:68px;--engagement-card-height:112px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));align-items:stretch;gap:16px;width:100%;max-width:1400px;margin:0 auto 20px}
    .engagement-row>.activity-rail,.engagement-row>.ranking-wrapper{min-width:0;height:100%;margin:0;box-sizing:border-box}
    .activity-rail,.engagement-row .ranking-section{display:flex;flex-direction:column;padding:18px;border:1px solid rgba(255,255,255,.08);border-radius:18px;background:rgba(16,22,35,.9);box-sizing:border-box}
    .activity-rail{max-width:1400px;margin:0 auto 20px;box-shadow:0 15px 40px rgba(0,0,0,.25)}
    .engagement-row .ranking-section{height:100%;box-shadow:0 15px 40px rgba(0,0,0,.25)}
    .activity-head,.engagement-row .ranking-header{display:flex;align-items:flex-end;justify-content:space-between;gap:12px;min-height:54px;margin:0 0 14px}
    .activity-head>div,.engagement-row .ranking-heading{display:grid;gap:6px;min-width:0}
    .activity-kicker,.engagement-row .ranking-kicker{color:#7df1c2;font-size:10px;font-weight:800;line-height:14px;letter-spacing:.14em}
    .activity-head h2,.engagement-row .ranking-title{margin:0;color:#e9edf5;font-size:18px;font-weight:700;line-height:28px}
    .activity-mine{border:1px solid rgba(111,167,255,.28);border-radius:999px;padding:7px 10px;background:rgba(111,167,255,.07);color:#a9c8ff;font-size:11px;line-height:18px;cursor:pointer;white-space:nowrap}
    .engagement-row .ranking-controls{min-height:34px;justify-content:flex-end;gap:6px}
    .activity-grid,.engagement-row .ranking-list{display:grid;grid-template-columns:1fr;align-items:start;gap:10px}
    .activity-card,.engagement-row .ranking-item{min-width:0;min-height:var(--engagement-card-height,112px);padding:10px;border:1px solid rgba(255,255,255,.075);border-radius:13px;background:rgba(255,255,255,.025);box-sizing:border-box;cursor:pointer}
    .activity-card{display:grid;grid-template-columns:var(--engagement-thumb,68px) minmax(0,1fr);gap:10px;transition:.2s ease}
    .activity-card:hover{transform:translateY(-2px);border-color:rgba(111,167,255,.38);background:rgba(111,167,255,.055)}
    .activity-card img,.activity-image-fallback,.engagement-row .ranking-thumb{width:var(--engagement-thumb,68px);height:var(--engagement-thumb,68px);border-radius:9px;background:linear-gradient(135deg,#223248,#101925);box-sizing:border-box}
    .activity-card img{object-fit:cover}
    .activity-image-fallback{display:grid;place-items:center;color:#70839a;font-size:20px}
    .activity-meta{margin-bottom:3px;color:#91a5be;font-size:11px;line-height:1.5;overflow-wrap:anywhere}
    .activity-meta strong{color:#7df1c2}
    .activity-card h3,.engagement-row .ranking-title-text{margin:0;color:#eef3f8;font-size:13px;font-weight:700;line-height:1.5;overflow-wrap:anywhere}
    .activity-card h3{overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical}
    .activity-card p{margin:6px 0 0;color:#c5d2e2;font-size:14px;line-height:1.65;white-space:pre-wrap;overflow-wrap:anywhere}
    .activity-empty{grid-column:1/-1;padding:12px;color:#8191a7;font-size:12px;text-align:center}
    .engagement-row .ranking-item{grid-template-columns:minmax(0,1fr) auto;gap:10px}
    .engagement-row .ranking-left{align-items:center;gap:10px}
    .engagement-row .ranking-thumb{flex-basis:var(--engagement-thumb)}
    .engagement-row .ranking-rank{width:18px;flex:0 0 18px;font-size:14px}
    .engagement-row .ranking-meta{flex-wrap:nowrap;gap:6px;font-size:10px;white-space:nowrap}
    .engagement-row .ranking-stat{font-size:10px}
    .engagement-row .ranking-meta .action-btn{min-width:30px;height:30px;justify-content:center;padding:3px 5px;font-size:11px;gap:3px}
    .engagement-row .ranking-meta .remove-news-btn{width:30px}
    .activity-more-row,.engagement-row .ranking-footer{display:flex;justify-content:center;align-items:center;margin-top:auto;padding-top:14px;min-height:34px}
    .activity-more,.engagement-row .ranking-footer .ranking-toggle{min-height:34px;margin:0;border:1px solid rgba(125,241,194,.25);border-radius:999px;padding:7px 16px;background:rgba(125,241,194,.055);color:#aeeed6;font-size:11px;line-height:18px;box-sizing:border-box;cursor:pointer}
    .activity-more:hover,.engagement-row .ranking-footer .ranking-toggle:hover{background:rgba(125,241,194,.11)}
    .activity-more[hidden]{display:none}
    .engagement-row .ranking-body{flex:0 0 auto}
    ...
    @media(max-width:1280px){.engagement-row{grid-template-columns:1fr;width:calc(100% - 20px);margin-inline:auto}.engagement-row>.activity-rail,.engagement-row>.ranking-wrapper{margin:0}.engagement-row .ranking-item{grid-template-columns:minmax(0,1fr) auto}}
    @media(max-width:680px){.engagement-row .ranking-header{align-items:flex-start;flex-direction:column}.engagement-row .ranking-item{grid-template-columns:minmax(0,1fr)}.engagement-row .ranking-meta{padding-left:28px;flex-wrap:wrap;white-space:normal}}
    @media(max-width:860px){.header-top-actions{grid-template-columns:minmax(0,1fr) auto auto auto!important;column-gap:7px}.activity-grid{grid-template-columns:1fr}.activity-head{align-items:flex-start;flex-wrap:wrap}.activity-rail{margin-inline:10px}.engagement-row .activity-rail{margin-inline:0}}
