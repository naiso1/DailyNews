...
function injectActivityStyles() {
  ...
  style.textContent = `
    ...
    .activity-head{display:flex;align-items:flex-end;justify-content:space-between;gap:15px;margin-bottom:12px}.activity-kicker{color:#7df1c2;font-size:9px;font-weight:800;letter-spacing:.14em}.activity-head h2{margin:2px 0 0;color:#e9edf5;font-size:17px}.activity-head p{margin:0;color:#8191a7;font-size:10px}.activity-mine{border:1px solid rgba(111,167,255,.28);border-radius:999px;padding:6px 10px;background:rgba(111,167,255,.07);color:#a9c8ff;font-size:10px;cursor:pointer;white-space:nowrap}
    .activity-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.activity-card{display:grid;grid-template-columns:68px minmax(0,1fr);gap:10px;min-width:0;padding:9px;border:1px solid rgba(255,255,255,.075);border-radius:13px;background:rgba(255,255,255,.025);cursor:pointer;transition:.2s ease}.activity-card:hover{transform:translateY(-2px);border-color:rgba(111,167,255,.38);background:rgba(111,167,255,.055)}
  ...
  if (filters) {
    ...
    rail.innerHTML = `
      <div class="activity-head">
        ...
        <p>コメントが付いた記事を3件だけ表示。新着ニュースの並びは変えません。</p>
...
function renderRecentActivity() {
  ...
  const rows = activityState.recent
...
async function loadActivity() {
  ...
  try {
    const [recent, notices] = await Promise.all([
      activityApi("/activity/recent?limit=6"),
