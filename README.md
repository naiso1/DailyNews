...
- 5地域。製品・技術に加え、外装開発の判断に役立つデザイン・市場・規制・材料・競合のトレンドを選定。各地域最大10件、うちトレンド最大4件。画面で記事区分を切り替えられる。
- 元記事の画像は利用する。外装の企画画像はexaBase連携で最新日の先頭最大4案を対象に生成する。同じ日の再実行で5案目以降へ広げず、有料画像APIへ自動で切り替えない。認証切れなどで利用できない場合も文章を公開する。
